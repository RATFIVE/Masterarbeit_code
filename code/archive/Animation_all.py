...
def animation_plot(
    ...
    if "pressure_msl" in df_weather_time.columns:
        ...
        if pressure_msl_grid is not None:
            ...
            plt.clabel(cs, inline=True, fontsize=8, fmt="%.0f hPa")
...
def run_initial_frame(timepoints):
    ...
    ax1_in.legend()
    ax1_in.legend(loc='center left', bbox_to_anchor=(0.95, 0.9)) # x, y
    ...
    fig_in.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(0.90, 0.59), frameon=True, title="Legend")
...
def update_animation(timepoints, frame_idx, fig:plt.Figure, axs:plt.Axes, gs:mpl.gridspec.GridSpec):
    ...
    fig.clf()
    gs = gridspec.GridSpec(2, 1, height_ratios=[1, 2], figure=fig, wspace=0.2, hspace=0.4)
    ...
    ax1.legend(loc='center left', bbox_to_anchor=(0.95, 0.9)) # x, y
    ...
    cbar_ax = fig.add_axes([0.25, 0.05, 0.5, 0.03]) # links, unten, breite, höhe
    fig.colorbar(heatmap, cax=cbar_ax, orientation="horizontal").set_label("water level (m)")
    legend_elements = [Line2D([0], [0], color='green', marker='*', linestyle='', markersize=15, label='Flensburg')]
    legend_elements.append(Line2D([0], [0], color='black', lw=4, marker=r'$\rightarrow$', label='wind direction', linestyle=''))
    legend_elements.append(Line2D([0], [0], color='grey', lw=4, marker=r'$\rightarrow$', label='ocean current', linestyle=''))
    fig.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(0.90, 0.59), frameon=True, title="Legend")
...
def start_full_animation(timepoints, fps=6, save_as_gif=False):
    ...
    def update(frame_idx):
        ...
        fig.suptitle(f"Zeitpunkt: {current_time}", fontsize=18)
...
if __name__ == "__main__":
    ...
    #                 ]
    print('\nErste 10 Zeitpunkte:')
    print(timepoints_all[:10]) # erste 10 Zeitpunkte
    print('\nLetzte 10 Zeitpunkte:')
    print(timepoints_all[-10:]) # letzte 10 Zeitpunkte

    print("\nRunning animation for all sturm surges...")
    start_full_animation(timepoints_all, fps=6, save_as_gif=True)
