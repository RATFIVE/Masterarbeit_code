...
class RecallLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        predict_onehot = torch.zeros((N, C, last_size)).to(self.device) # (N, 1, *) ==> (N, C, *)
        predict_onehot.scatter_(1, predict, 1) # (N, C, *)
        target_onehot = torch.zeros((N, C, last_size)).to(self.device) # (N, 1, *) ==> (N, C, *)
...
if __name__ == '__main__':
    ...
    y_target = torch.Tensor([[0, 1], [1, 0]]).long().to(device)
